...
def get_loader(train=False, val=False, test=False, test_dev=False):
    """ Returns a data loader for the desired split """
    split = VQA(
        utils.path_for(train=train, val=val, test=test, test_dev=test_dev, question=True),
        utils.path_for(train=train, val=val, test=test, test_dev=test_dev, answer=True),
        config.preprocessed_trainval_path if not (test or test_dev) else config.preprocessed_test_path,
...
class VQA(data.Dataset):
    ...
    def get_id_ques_id_map(self):
        self.id_ques_id_map = {i:ques_id for i, ques_id in enumerate(self.question_ids)}

    def get_id_ans_map(self):
        self.id_ans_map = {i:ans_str for i,ans_str in enumerate(self.answer_to_index)}

    @property
    def id_2_ques_id_map(self):
        return self.id_ques_id_map

    @property
    def id_2_ans_map(self):
        return self.id_ans_map
